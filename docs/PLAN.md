...
| Route | Notes |
|---|---|
| `GET /` | **the one page**: hero art + 3-step instructions + the feed + search box + footer "for agents" links. `?sort=hot|new|top|rising|controversial|updated&t=hour|day|week|month|year|all&page=`. Each of `rising`, `controversial`, `updated` and `upcoming` sits behind a flag of its own name in `MOD_FLAGS`; switched off (as they are now: too thin a signal to be worth a tab) they lose their tab and chip and `?sort=` falls back to `hot`, everywhere including the API and MCP |
...
| `GET /upcoming` | **up and coming**: listed repos whose declared `status` is `idea`, `prototype`, `works-on-my-machine`, or `alpha`, sorted by rising; also a tab on `/` and `/upcoming.xml`. Behind the `upcoming` flag: off, the route is a 404 and drops out of the sitemap (the `upcoming-week` award still runs) |
| `GET /upvoted` | **slop you upvoted** (login): every repo the session user ever upvoted, newest vote first, in any status so a removal shows its chip rather than vanishing. Per-user rows, so it skips the shared edge cache; `noindex` |
...
- **Cast art (done 2026-09-12).** Five painted masters in `art/` that are *not* generated from code: `cast-lagoon.png` (the four critics on a beach, Sloptrawler behind) and `plate-<name>.png` (a framed locket per critic, name engraved in the picture). `art/cast.py` cuts them into `public/cast/`: the wide crew shot (also /balcony's OpenGraph card), a face per critic for the box seats, the ship's own portrait, and the four lockets with the cream paper keyed out to transparency. `/lockets.js` tilts a hovered locket toward the pointer and moves the point light in the `#locketRelief` SVG filter so the frame's relief lights up — desktop only, off under `prefers-reduced-motion`, and the pictures stand alone without it.
...
- **Three lanes.** Paid jumpers, then the free (human) line, then the Cap'm's trawled finds. The first two are the FIFO lines below. The third is separate on purpose: trawled repos are our own work, so `src/jobs/scan.ts` scans them in a lane with its own per-tick allowance and sends their AI checks to OpenRouter — they never consume a free scan slot or a Workers AI neuron, and they run only while `OPENROUTER_API_KEY` is set. A trawled repo that commits `slopscore.md` is promoted to the free line by the sweep the moment code search sees the file.
