...
// No model reads the message, and the requester only ever sees canned replies. A trawled listing is hidden immediately
// and deleted for good after TAKEDOWN_GRACE unless a moderator restores it, up to TAKEDOWN_AUTO_PER_DAY a day site-wide;
// beyond that, requests queue for a human. The message stays in the mod console.
...
import { validateTakedown, hideTrawled, takedownPlan, TAKEDOWN_GRACE } from "../lib/virtual";
import { notify } from "./contact";
import { Layout } from "../views/layout";

export const takedown = new Hono<AppEnv>();

const GRACE_DAYS = Math.round(TAKEDOWN_GRACE / 86400);

const CANNED = {
  removed: `Hidden. The listing is off the site as of now, and it is deleted for good in ${GRACE_DAYS} days unless a moderator sees a reason to keep it, which is rare. It won't be re-added: we keep only the repo's name so the trawl never brings it back. Sorry for the bother.`,
    ...
    <Layout meta={{ title: `Takedown · ${name} — SlopScore`, noindex: true }} user={user} url={url}>
      <section class="wrap narrow" style="padding:0">
        ...
        {!r ? <p>Nothing is listed as <code>{name}</code>, so there's nothing to take down.</p>
          ...
          : r.status === "delisted" || (r.status === "hidden" && r.queue_reason === "takedown") ? (done ? null : <p>{CANNED.removed}</p>)
          : (
            <>
              <p><a href={`/r/${r.full_name}`}>{r.full_name}</a> was listed by the Cap'm's trawl; its owner never submitted it. If you'd like it gone, say so below. A trawled listing is hidden right away, deleted for good {GRACE_DAYS} days later, and never comes back.</p>
...
takedown.post("/:owner/:name/takedown", async (c) => {
  ...
  if (r.status === "delisted" || (r.status === "hidden" && r.queue_reason === "takedown")) return reply("removed");
  ...
  const plan = takedownPlan(auto, now());
  const ins = await db.prepare("INSERT INTO takedowns (repo_id, full_name, user_id, login, contact, message, ip_hash, outcome, settle_at) VALUES (?,?,?,?,?,?,?,?,?)")
    .bind(r.id, r.full_name, who?.id ?? null, who?.login ?? null, v.contact, v.message, ip, plan.outcome, plan.settle_at).run();
  const id = Number(ins.meta.last_row_id);
  // Hidden now, deleted later: the listing leaves the site on the spot, and the step nobody can undo waits for
  // the grace to pass (settleTakedowns, in the daily round) so a moderator can put a bad-faith request right.
  if (auto) await hideTrawled(db, r.id);
  // The public log records that a takedown happened, never what the message said.
  await logAction(db, { actor: "system", role: "system", action: auto ? "takedown" : "takedown-queued", targetType: "repo", targetId: r.id, label: r.full_name, note: auto ? `trawled listing hidden on request; removed for good in ${GRACE_DAYS} days` : "daily automatic limit reached; queued for a human" });
  c.executionCtx.waitUntil(
    notify(c.env, { id, login: who?.login ?? "someone not logged in", subject: `takedown (${auto ? "hidden" : "queued"})`, body: v.message + (v.contact ? `\n\nContact: ${v.contact}` : ""), repo: r.full_name, anchor: "td" })
