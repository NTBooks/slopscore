...
// {"upvote", "reason"}, and the reason is stored privately. The worst a hostile README can do is earn one
// half-weight upvote; it can never make a critic say anything in public.
...
import {
  CRITICS, CRITICS_MODEL_DEFAULT, CRITIC_DAILY_CAP, CRITIC_PER_RUN, VERDICT_SCHEMA,
  criticSystemPrompt, criticUserPrompt, dayStart, parseVerdict, type Critic, type Verdict,
} from "../lib/critics";
...
export async function runCritics(env: Env, opts: { n?: number; dry?: boolean } = {}): Promise<CriticsResult> {
  ...
  const perRun = Math.min(Math.max(1, Math.floor(Number(opts.n) || CRITIC_PER_RUN)), CRITIC_DAILY_CAP);
  ...
  for (const c of CRITICS) {
    ...
    const budget = Math.min(perRun, CRITIC_DAILY_CAP - done);
    if (budget <= 0) { mine.note = `today's ${CRITIC_DAILY_CAP} are done`; continue; }
