...
pages.get("/u/:login", async (c) => {
  ...
  // The header as well as the meta tag: /u/x.json and /u/x.md have no <head> to carry the tag in.
  if (!optedIn) c.header("X-Robots-Tag", "noindex, follow");
  return feedPage(c, {
    noindex: !optedIn,
...
pages.get("/about", (c) => {
  ...
  const md = [
    ...
    "The leaderboard is the small version of a bigger ask. What the site is actually for is in the [manifesto](/manifesto): a self-reported disclosure panel on every kind of AI-made media, and a [directory](/schemas) of the ones that exist so far. Teaching a course or running a hackathon? [Start here](/campus).", "",
    "## Meet Schnitzel", "",
    ...
    "## Trawled listings", "",
    "To fill the trough early, the Cap'm goes on a truffle trawl. He reads public repos whose owners say they were vibe coded or built with an AI tool, keeps the ones with a permissive license (MIT, Apache-2.0, BSD, ISC, 0BSD, Unlicense or CC0) that nobody submitted, and lets a few into the trough each day. Their pages say so at the top, their paperwork is his best guess from GitHub data, they sort below every repo that opted in, they stay out of the RSS feed, and they can't win awards. " + (trawlIndexed(c.env) ? "They are in the sitemap on purpose: searching for your own repo is how you find the listing, and the button that removes it. " : "They are kept out of search engines on this deploy: readable and votable, but never indexed. ") + "The owner can replace his paperwork with their own `slopscore.md`, or remove the listing in one click. Anyone who can't log in as the owner can request a takedown without logging in: the listing is hidden right away and deleted for good three days later, unless a moderator finds the request was made in bad faith and restores it. The trawl stops for good once enough repos opt in.", "",
...
pages.get("/r/:owner/:name", async (c) => {
  ...
  // engines get it; the header covers .json and .md as well as the HTML head's meta tag below.
  const hideFromSearch = r.source === "trawl" && !trawlIndexed(c.env);
