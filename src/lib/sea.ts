...
// in:description queries at another. The voyage is the shape of the clock: the trawl sails once an hour
// (cron `7 * * * *`, plus the odd chase), so an hour is one round trip, and where she is in it is what
// time it is. Once the day's budget is landed she stays tied up until the next UTC day, and says so.
//
...
export const VOYAGE_PERIOD = 3600;

/**
 * Where the legs end, as a fraction of the voyage. Standing out takes a while, the grounds take most of
 * it, running home is quicker than going, and she lies at the Trough being emptied until the next sailing.
...
export interface Voyage {
  ...
  t: number;
  /** 0..1 through the voyage. */
  phase: number;
  /** Whole voyages since `last`: how far the facts the chart was drawn from have drifted. */
  rolled: number;
  /** She has missed two sailings. The day's catch is landed (see `spent` in voyageText), the expedition is
   *  over (TRAWL_STOP_AT), or something is wrong. */
  laidUp: boolean;
}
 ...
 *
 * `last` comes out of crawl_state and can be hours or days stale, so the drift is measured rather
 * than ignored: `rolled` says how many whole voyages have been missed, and the phase is taken from
 * the remainder. Without that a stale row would have her sailing a day's worth of the leg in one
 * hour, off the end of the chart. A clock running behind the server's is clamped rather than allowed
...
/**
 * The cursor this hour, corrected for how stale the facts are.
 *
 * autoTrawl and trawl each advance trawl:cursor by exactly one per run, so a cached page can work out
 * where the counter has got to rather than showing last hour's ground. It is only right if she actually
 * sailed on each of those hours; once the day's budget is landed, or TRAWL_STOP_AT has stopped her, the
 * counter does not move, which is why laidUp exists and why the caption stops naming a ground once it
 * is set.
 */
export const rolledCursor = (cursor: number, v: Voyage): number => Math.floor(Number(cursor) || 0) + v.rolled;

/** What the chart says she is doing. Never claims she is fishing when she is tied up. `spent` means the
 *  day's budget is landed, which is the usual reason for her to be tied up and not a fault. */
export function voyageText(v: Voyage, g: Ground, spent = false): string {
  ...
  if (spent) return "The Sloptrawler is tied up at the Trough. Today's catch is landed; she sails again after midnight UTC.";
  if (v.laidUp) return "The Sloptrawler is laid up at the Trough. She has not been out for hours.";
  switch (v.leg) {
